...
const calcularNotaConsultor = document.getElementById('calcularNotaConsultor');
const operacaoMetaInput = document.getElementById('operacaoMeta');
const operacaoPessoasInput = document.getElementById('operacaoPessoas');
const operacaoCampanhaInput = document.getElementById('operacaoCampanha');
const operacaoFalta = document.getElementById('operacaoFalta');
const operacaoMedia = document.getElementById('operacaoMedia');
const operacaoPorConsultor = document.getElementById('operacaoPorConsultor');
...
const fetchContemplados = async () => {
  try {
    ...
    state.contemplados = Array.isArray(payload.rows) ? payload.rows : [];
    if (state.user) {
      renderContemplados();
    }
  } catch (error) {
...
const renderContemplados = () => {
  ...
  if (!state.contemplados.length) {
    const li = document.createElement('li');
    li.className = 'robot-file robot-file--empty';
    li.textContent = 'Nenhum contemplado encontrado na planilha.';
...
const calcularMediaConsultor = () => {
  ...
  const inputs = consultorForm.querySelectorAll('input[type="number"]');
  const values = Array.from(inputs).map((input) => Number(input.value) || 0);
  const total = values.reduce((acc, value) => acc + value, 0);
  const media = values.length ? (total / values.length).toFixed(1) : '0';
  consultorResultado.textContent = `Média geral: ${media}`;
};

const normalizeNumberInput = (value) =>
  parseCurrencyToNumber(value?.toString().replace(/\s+/g, '') || '');

const renderOperacaoResumo = () => {
  if (!operacaoMetaInput || !operacaoPessoasInput) {
    return;
  }
  const meta = normalizeNumberInput(operacaoMetaInput.value);
  const pessoas = Math.max(Number(operacaoPessoasInput.value) || 1, 1);
  const currentSales = state.operation.currentSales;
  const falta = Math.max(meta - currentSales, 0);
  const media = pessoas ? currentSales / pessoas : 0;
  const porConsultor = pessoas ? falta / pessoas : 0;

  if (operacaoFalta) {
    operacaoFalta.textContent = formatCurrency(falta);
  }
  if (operacaoMedia) {
    operacaoMedia.textContent = formatCurrency(media);
  }
  if (operacaoPorConsultor) {
    operacaoPorConsultor.textContent = formatCurrency(porConsultor);
  }
  if (operacaoCampanhaFalta && operacaoCampanhaInput) {
    const goal = Number(operacaoCampanhaInput.value) || state.operation.campaignGoal;
    const missing = Math.max(goal - state.operation.campaignCurrent, 0);
    operacaoCampanhaFalta.textContent = `${missing} pts`;
  }
  if (operacaoMetaConsultor) {
    operacaoMetaConsultor.textContent = formatCurrency(meta / pessoas);
  }
  if (operacaoConsultores) {
    operacaoConsultores.textContent = pessoas.toString();
  }
};
...
const setActiveChamadosTab = (tabId) => {
  ...
  chamadosContents.forEach((content) => {
    content.classList.toggle('is-active', content.dataset.chamadosContent === tabId);
  });
};
...
const updateChamadosByRole = (role) => {
  ...
  chamadosTabs.forEach((tab) => {
    const allowed = (tab.dataset.roles || '')
      .split(',')
      .map((value) => value.trim())
      .filter(Boolean);
    const visible = allowed.length ? allowed.includes(normalizedRole) : true;
    tab.style.display = visible ? '' : 'none';
  });
  chamadosContents.forEach((content) => {
    const allowed = (content.dataset.roles || '')
      .split(',')
      .map((value) => value.trim())
      .filter(Boolean);
    const visible = allowed.length ? allowed.includes(normalizedRole) : true;
...
const updateMenuByRole = (role) => {
  ...
  if (!activeVisible && visibleItems.length) {
...
if (calcularNotaConsultor) {
  calcularNotaConsultor.addEventListener('click', () => {
    calcularMediaConsultor();
  });
}

if (consultorForm) {
  consultorForm.addEventListener('input', () => {
    calcularMediaConsultor();
  });
}

if (operacaoMetaInput) {
  operacaoMetaInput.addEventListener('input', renderOperacaoResumo);
}
if (operacaoPessoasInput) {
  operacaoPessoasInput.addEventListener('input', renderOperacaoResumo);
}
if (operacaoCampanhaInput) {
  operacaoCampanhaInput.addEventListener('input', renderOperacaoResumo);
}
